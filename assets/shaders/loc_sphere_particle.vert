...
void main(void)
{
    ...
    float nx = gl_Vertex.x + xoff;
    float ny = gl_Vertex.y + yoff;
    
//	gl_Vertex.x += xoff;
//	gl_Vertex.y += yoff;
	gl_Position = gl_ModelViewProjectionMatrix * vec4(vec3(nx, ny, gl_Vertex.z), 1.0);
	...
	gl_PointSize =  12.0 + (pointPhase * 4.0 - 2.5);
}